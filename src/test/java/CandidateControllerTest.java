...
public class CandidateControllerTest {
    ...
    public void whenRequestVacancyPageThenGetPageWithVacancy() {
        var candidate = new Candidate(1, "test1", "desc1", now(),  1, 2);
        var expectedCandidate = Optional.ofNullable(candidate);
        ...
        assertThat(actualCandidates).isEqualTo(expectedCandidate.get());
    }
    ...
    public void whenSomeExceptionThrownThenGetErrorPageWithMessage2() {
        ...
        var model = new ConcurrentModel();
        var view = candidateController.getById(model, 0, session);
        var actualExceptionMessage = model.getAttribute("message");
    ...
    @Test
    public void whenPostVacancyWithFileThenSameDataAndRedirectToVacanciesPage2() throws Exception {
        var candidate = new Candidate(1, "test1", "desc1", now(), 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var candidateArgumentCaptor = ArgumentCaptor.forClass(Candidate.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
    ...
    @Test
    public void whenPostVacancyWithFileThenSameDataAndRedirectToVacanciesPage3() throws Exception {
        var candidate = new Candidate(1, "test1", "desc1", now(), 1, 2);
    ...
    @Test
    public void whenSomeExceptionThrownThenGetErrorPageWithMessage4() {
        var expectedException = new RuntimeException("Резюме с указанным идентификатором не найдено");
